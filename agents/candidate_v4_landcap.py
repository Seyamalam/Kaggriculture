"""Kaggriculture v4: four-sheep policy with a validated two-quadrant cap.
...
def _unit_actions(obs: dict[str, Any]) -> tuple[list[Any], list[list[Any]]]:
    ...
    for index in list(remaining_units):
        ...
        animal_sale_load = int(inv.get("WOOL", 0)) + int(inv.get("FERTILIZER", 0))
        final_day = day >= 29
    ...
    for _ in range(min(sheep_in_shed, len(empty_pastures))):
        assign_pickup("SHEEP", 1)
...
def _market_actions(obs: dict[str, Any]) -> list[list[Any]]:
    ...
    carried_sheep = sum(int(inv.get("SHEEP", 0)) for inv in inventories)
    ...
    feed_reserve = 0 if day >= 29 else max(2, placed_sheep * 2)
    ...
    # Stage one sheep behind the opening land purchase, a second as fertilizer
    # cash arrives, and the final pair after the first wool cycle.
    sheep_in_system = placed_sheep + carried_sheep + int(shed.get("SHEEP", 0))
    desired_sheep = 1 if day < 5 else (2 if day < 7 else 4)
    if day <= 14 and sheep_in_system < desired_sheep and len(orders) < 10:
        reserve = 250 + max(2, sheep_in_system + 1) * 25 * 2
        if money >= SHEEP_COST + reserve:
            orders.append(["BUY_ANIMAL", "SHEEP", 1])
            money -= SHEEP_COST
            sheep_in_system += 1

    wheat_stock = int(shed.get("WHEAT", 0)) + carried_wheat
    feed_target = 0 if day >= 29 else max(4, sheep_in_system * 2)
    if sheep_in_system and wheat_stock < feed_target and len(orders) < 10:
    ...
    workload = active + seed_stock + placed_sheep * 3
    ...
    capacity = max(0, min(75, 25 * unlocked + pending_tiles) - len(SHEEP_TILES) - 2)
    gap = min(max(0, capacity - active - seed_stock), max(0, 6 - seed_stock))
    if gap > 0 and len(orders) < 10 and day <= 21:
        scores = _crop_scores(obs)
        ...
        if crop is not None and spendable >= CROPS[crop]["seed"]:
            count = min(6, gap, int(spendable // CROPS[crop]["seed"]))
            if count > 0:
